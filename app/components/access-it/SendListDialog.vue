...
function send() {
  ...
  if (!email) {
    toast.error('The on-site list notification is disabled in Settings.')
    return
  }
  toast.success(`On-site list prepared for ${email.to}`, { description: 'Saved to the demo outbox. No email sent.' })
      ...
      <DialogContent class="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl outline-hidden sm:p-7">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <DialogDescription class="mt-1.5 text-sm text-slate-500">Prepare a list of everyone currently on site for the nominated emergency contact. This demo saves it to the outbox; no email is sent.</DialogDescription>
          </div>
        ...
        <form class="mt-6 space-y-5" @submit.prevent="send">
          <label class="block space-y-2">
            ...
            <span class="block text-xs text-slate-500">{{ siteProfile.emergencyContactName }} is the nominated emergency contact.</span>
          </label>
          ...
          <p v-if="!automationActive" class="rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">The on-site list notification is disabled. Enable it in Access IT settings to prepare the list.</p>
          ...
          <div class="flex justify-end gap-3">
            ...
            <button type="submit" :disabled="!automationActive" class="flex h-10 items-center gap-2 rounded-lg bg-soter-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-soter-700 disabled:cursor-not-allowed disabled:opacity-50"><Send class="size-4" /> Prepare demo list</button>
          </div>
        </form>
      </DialogContent>
    </DialogPortal>
